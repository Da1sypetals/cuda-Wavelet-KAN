                           ...
                           int out_feats, int numThreads)
{
    ...
    if (idx < numThreads)
    {
        int ibatch = idx / (in_feats * out_feats);
        int iin = (idx / out_feats) % in_feats;
        int iout = idx % out_feats;

        float gout_val = gout[ibatch][iout];
        float x_val = x[ibatch][iin];
        float s = scale[iin][iout];
        float b = bias[iin][iout];
        float w = weight[iin][iout];
        ...
        grad_x[ibatch][iin][iout] = gout_val * g_x;
        grad_s_expand[ibatch][iin][iout] = gout_val * g_s;
        grad_b_expand[ibatch][iin][iout] = gout_val * g_x;
        grad_w_expand[ibatch][iin][iout] = gout_val * g_w;
    }
}
