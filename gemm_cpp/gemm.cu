...
__global__ void fwd_gemm_kernel(int M, int N, int K,
                                float *A, float *B,
                                float *S, float *W,
                                float *C)
{
    ...
    S = &S[OFFSET(0, blockIdx.x * BN, N)];
    W = &W[OFFSET(0, blockIdx.x * BN, N)];
    ...
    float Ss_cache[2][TN] = {0.}; // double buffering
    float Ws_cache[2][TN] = {0.}; // double buffering
...
#pragma unroll
        for (int k = 0; k < BK; k += b_tile_stride)
        {
            ...
            FETCH_FLOAT4(Ss[write_idx][b_tile_row + k][b_tile_col]) =
                FETCH_FLOAT4(S[OFFSET(b_tile_row + k, b_tile_col, N)]);

            FETCH_FLOAT4(Ws[write_idx][b_tile_row + k][b_tile_col]) =
                FETCH_FLOAT4(W[OFFSET(b_tile_row + k, b_tile_col, N)]);
        }
        __syncthreads();

        A += BK;     // Start position of next tile block to be processed
        B += BK * N; // Start position of next tile block to be processed
        S += BK * N; // Start position of next tile block to be processed
        W += BK * N; // Start position of next tile block to be processed
...
#pragma unroll
            for (int n = 0; n < TN; n += 4)
            {
                ...
                FETCH_FLOAT4(Ss_cache[write_idx][n]) = FETCH_FLOAT4(Ss[write_idx][k][B_col]);
                FETCH_FLOAT4(Ws_cache[write_idx][n]) = FETCH_FLOAT4(Ws[write_idx][k][B_col]);
            }
#pragma unroll
            for (int m = 0; m < TM; ++m)
            {
#pragma unroll
                for (int n = 0; n < TN; ++n)
                {
                    float y2 = Ss_cache[write_idx][n] * Ss_cache[write_idx][n] *
                               (As_cache[write_idx][m] + Bs_cache[write_idx][n]) *
                               (As_cache[write_idx][m] + Bs_cache[write_idx][n]);

                    accum[m][n] += COEFF * Ws_cache[write_idx][n] * (y2 - 1) * expf(-0.5f * y2);
                }
            }
        }
        write_idx ^= 1;
...
#pragma unroll
        for (int n = 0; n < TN; n += 4)
        {
            ...
            load_a_cache[0] = accum[m][n];
            load_a_cache[1] = accum[m][n + 1];
            load_a_cache[2] = accum[m][n + 2];
            load_a_cache[3] = accum[m][n + 3];
            // printf("load_a_cache = (%f, %f, %f, %f)",
            //        load_a_cache[0],
            //        load_a_cache[1],
            //        load_a_cache[2],
            //        load_a_cache[3]);
                       ...
                       float *weight, float *result)
{
    ...
    dim3 grid(CEIL_DIV(out_feats, BN), CEIL_DIV(batch_size, BM)); // note: change M and N here
    fwd_gemm_kernel<BM, BN, BK, TM, TN>
        <<<grid, block>>>(batch_size, out_feats, in_feats, x, bias, scale, weight, result);
}
