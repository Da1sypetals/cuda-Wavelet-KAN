...
__global__ void gemm_u(int M, int N, int K, float *A, float *B, float *C)
{
...
#pragma unroll
                for (int n = 0; n < TN; ++n)
                {
                    float y2 = (As_cache[write_idx][m] + Bs_cache[write_idx][n]) *
                               (As_cache[write_idx][m] + Bs_cache[write_idx][n]);
                    accum[m][n] += y2 * expf(-0.5f * y2);
                }
            }
        }
        write_idx ^= 1;
...
void gemm_u_launcher(int batch_size, int in_feats, int out_feats,
                     float *x, float *bias, float *result)
{
    const int size = 16;
    ...
    gemm_u<BM, BN, BK, TM, TN>
        <<<grid, block>>>(batch_size, out_feats, in_feats, x, bias, result);
}
