                           ...
                           int batch_size, int batch_size_padded, int in_feats, int out_feats, int numThreads)
{
    ...
    s = __shfl_sync(ALL_THREADS_IN_WARP, s, 0);
    b = __shfl_sync(ALL_THREADS_IN_WARP, b, 0);
    w = __shfl_sync(ALL_THREADS_IN_WARP, w, 0);
...
{
    int batch_size_padded = DIVUP(batch_size, 32) * 32;
    ...
    fwd_kernel<<<blockSize, threadSize>>>(x, scale, bias, weight, result,
                                          batch_size, batch_size_padded, in_feats, out_feats, numThreads);
}
